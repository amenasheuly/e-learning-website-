    ...
    <section class="content content-light">
        <div class="container">
            ...
            <form action="register.php" method="post">
                ...
                <button class="btn btn-theme btn-default"><a href="../login/user_login.php">Login</a></button></p>
...
if(isset($_POST['submit'])){
    if($first_name&&$user&&$pass&&$last_name&&$email&&$repass&&$date&&$reemail&&$gender&&$fb&&$linkedin){
        if(strlen($user) >= 5 && strlen($user)<= 15 && strlen($pass)>= 6) {
            ...
            $query = "SELECT * FROM `users` WHERE `user` = '$user' OR `email` ='$email' LIMIT 1 ";
            ...
            if ($singleData) {
                ...
                if ($repass == $pass && $reemail == $email ) {
                    ...
                    $sql = "INSERT INTO `online_learning`.`users` (`id`, `first_name`, `user`, `pass`, `last_name`, `email`, `date`, `gender`, `fb`, `link`) VALUES ('', '$first_name', '$user', '$pass', '$last_name', '$email', '$date', '$gender', '$fb', '$linkedin')";
